...
while ($pos = mysqli_fetch_assoc($positions_result)) {
    ...
    if ($max_votes > 0) {
        foreach ($candidates as $cand) {
            if ($cand['vote_count'] === $max_votes) {
                ...
                break; // Since it's ordered DESC, we can safely exit early
            }
        }
    }
    ...
    $pos['max_votes'] = $max_votes;
    $pos['leaders_count'] = $leaders_count;
            ...
            <div class="result-candidate">
                ...
                <div class="result-candidate-info">
                    <div class="result-candidate-name">
                        ...
                        <?php if ($c['vote_count'] > 0 && $c['vote_count'] === $pos['max_votes']): ?>
                            <?php if ($pos['leaders_count'] > 1): ?>
                                <span class="badge" style="background-color: #f39c12; color: #fff; padding: 2px 6px; font-size: 0.75rem; border-radius: 4px; margin-left: 5px;">TIED</span>
                            <?php else: ?>
